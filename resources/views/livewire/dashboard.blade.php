                    ...
                    <div class="bg-white dark:bg-zinc-800 overflow-hidden shadow-lg rounded-lg p-6 flex flex-col justify-between">
                        ...
                        <div class="mt-6">
                            ...
                            @if (in_array($evento->id, $eventosInscritosIds))
                                <button wire:click="cancelarRegistro({{ $evento->id }})" class="mt-4 w-full text-center bg-red-600 text-white font-bold py-2 px-4 rounded-md hover:bg-red-700">
                                    Cancelar Registro
                                </button>
                            @else
                                <button wire:click="registrar({{ $evento->id }})" 
                                    @disabled($evento->max_participantes && $evento->participantes_count >= $evento->max_participantes)
                                    class="mt-4 w-full text-center bg-blue-600 text-white font-bold py-2 px-4 rounded-md hover:bg-blue-700 disabled:bg-gray-500 disabled:cursor-not-allowed">
                                    Registrarme
                                </button>
                            @endif
                        </div>
